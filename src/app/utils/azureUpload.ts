...
export const uploadFileToBlobStorage = async (file: any): Promise<string> => {
  console.log(file);
  try {
    ...
    const accountName = process.env.AZURE_ACCT_NAME as string;
    const accountKey = process.env.AZURE_ACCT_KEY as string;
...
export const uploadMultipleToBlobStorage = async (files: any) => {
  try {
    const accountName = process.env.AZURE_ACCT_NAME as string;
    const accountKey = process.env.AZURE_ACCT_KEY as string;
    const containerName = process.env.AZURE_CONTAINER_NAME as string;
